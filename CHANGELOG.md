...
## v1.6.9 (Unreleased)
 - **BREAKING**: Removed Raspberry Pi 3 support (ARM64/Pi 5 only)
 - Removed python3-rpi.gpio dependency, now uses python3-lgpio exclusively
 - Simplified build scripts and workflows to focus on Pi 5
 - Updated documentation to reflect Pi 5 only support

## v1.6.8 (Unreleased)
 - Added Raspberry Pi 5 support
 - Created GPIO compatibility layer (src/py/bbctrl/gpio_compat.py)
 - Updated setup script to detect Pi model and install appropriate GPIO libraries
 - Pi 5 uses python3-lgpio and libgpiod-tools
